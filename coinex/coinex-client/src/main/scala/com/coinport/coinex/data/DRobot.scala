...
case class DRobot(
  ...
  type Action = (DRobot, Option[RobotMetrics]) => (DRobot, Option[Any])
  ...
  private val HEADER = """
    ...
    (robot: DRobot, metrics: Option[RobotMetrics]) =>

  """

  // invoked by outter processor
  def action(metrics: Option[RobotMetrics] = None): (DRobot, Option[Any]) = {
  ...
  def addHandler(state: String)(handler: String): DRobot = {
  ...
  def setPayload(state: String, payload: Option[Any]): DRobot = {
    copy(statesPayload = statesPayload + (state -> payload))
  }

  def ->(state: String): DRobot = { copy(currentState = state) }
