...
final class Coinex(routers: LocalRouters) extends Actor with Logging {
  ...
  def receive = {
    LoggingReceive {
      ...
      case m: DoVerifyRealName => routers.userProcessor forward m
      case m: DoAddBankCard => routers.userProcessor forward m
      case m: DoDeleteBankCard => routers.userProcessor forward m
      ...
      case m: QueryApiSecretByToken => routers.apiAuthView forward m
      ...
      case m: QueryAllDetailReserve => routers.bitwayViews(m.currency) forward m
