 ...
 *
 * This file was auto generated by auto_gen_serializer.sh on Wed Mar 26 21:47:09 CST 2014
 */
...
class EventSerializer extends Serializer {
  ...
  lazy val s_42 = BinaryScalaCodec(QueryUserTransaction)
  lazy val s_43 = BinaryScalaCodec(QueryUserTransactionResult)
  lazy val s_44 = BinaryScalaCodec(OrderSubmissionInProgross)
  lazy val s_45 = BinaryScalaCodec(SendMailRequest)
  lazy val s_46 = BinaryScalaCodec(DoRegisterUser)
  lazy val s_47 = BinaryScalaCodec(DoRequestPasswordReset)
  lazy val s_48 = BinaryScalaCodec(DoResetPassword)
  lazy val s_49 = BinaryScalaCodec(DoSubmitOrder)
  lazy val s_50 = BinaryScalaCodec(DoDepositCash)
  lazy val s_51 = BinaryScalaCodec(DoRequestCashWithdrawal)
  lazy val s_52 = BinaryScalaCodec(DoConfirmCashWithdrawalSuccess)
  lazy val s_53 = BinaryScalaCodec(DoConfirmCashWithdrawalFailed)
  lazy val s_54 = BinaryScalaCodec(DoCancelOrder)
  lazy val s_55 = BinaryScalaCodec(OrderCashLocked)
  lazy val s_56 = BinaryScalaCodec(OrderCancelled)
  lazy val s_57 = BinaryScalaCodec(OrderSubmissionFailed)
  ...
  def toBinary(obj: AnyRef): Array[Byte] = obj match {
    ...
    case m: QueryUserTransaction => s_42(m)
    case m: QueryUserTransactionResult => s_43(m)
    case m: OrderSubmissionInProgross => s_44(m)
    case m: SendMailRequest => s_45(m)
    case m: DoRegisterUser => s_46(m)
    case m: DoRequestPasswordReset => s_47(m)
    case m: DoResetPassword => s_48(m)
    case m: DoSubmitOrder => s_49(m)
    case m: DoDepositCash => s_50(m)
    case m: DoRequestCashWithdrawal => s_51(m)
    case m: DoConfirmCashWithdrawalSuccess => s_52(m)
    case m: DoConfirmCashWithdrawalFailed => s_53(m)
    case m: DoCancelOrder => s_54(m)
    case m: OrderCashLocked => s_55(m)
    case m: OrderCancelled => s_56(m)
    case m: OrderSubmissionFailed => s_57(m)
  ...
  def fromBinary(bytes: Array[Byte],
    ...
    case Some(c) if c == classOf[QueryUserTransaction.Immutable] => s_42.invert(bytes).get
    case Some(c) if c == classOf[QueryUserTransactionResult.Immutable] => s_43.invert(bytes).get
    case Some(c) if c == classOf[OrderSubmissionInProgross.Immutable] => s_44.invert(bytes).get
    case Some(c) if c == classOf[SendMailRequest.Immutable] => s_45.invert(bytes).get
    case Some(c) if c == classOf[DoRegisterUser.Immutable] => s_46.invert(bytes).get
    case Some(c) if c == classOf[DoRequestPasswordReset.Immutable] => s_47.invert(bytes).get
    case Some(c) if c == classOf[DoResetPassword.Immutable] => s_48.invert(bytes).get
    case Some(c) if c == classOf[DoSubmitOrder.Immutable] => s_49.invert(bytes).get
    case Some(c) if c == classOf[DoDepositCash.Immutable] => s_50.invert(bytes).get
    case Some(c) if c == classOf[DoRequestCashWithdrawal.Immutable] => s_51.invert(bytes).get
    case Some(c) if c == classOf[DoConfirmCashWithdrawalSuccess.Immutable] => s_52.invert(bytes).get
    case Some(c) if c == classOf[DoConfirmCashWithdrawalFailed.Immutable] => s_53.invert(bytes).get
    case Some(c) if c == classOf[DoCancelOrder.Immutable] => s_54.invert(bytes).get
    case Some(c) if c == classOf[OrderCashLocked.Immutable] => s_55.invert(bytes).get
    case Some(c) if c == classOf[OrderCancelled.Immutable] => s_56.invert(bytes).get
    case Some(c) if c == classOf[OrderSubmissionFailed.Immutable] => s_57.invert(bytes).get
