...
class CryptoCurrencyTransferDepositLikeHandler(currency: Currency, outputPort: CryptoCurrencyTransactionPort, tx: CryptoCurrencyTransaction, timestamp: Option[Long])(implicit env: TransferEnv) extends CryptoCurrencyTransferHandler {
  ...
  if (currency != null && outputPort != null && tx != null) {
    ...
    if (outputPort.currency.isDefined && outputPort.currency.get == Currency.Cny) {
      transferHandler.put(AccountTransfer(transferId, outputPort.userId.get, tx.txType.get, outputPort.currency.get, outputPort.internalAmount.get, Confirming, getTimestamp(), getTimestamp(), address = Some(outputPort.address), txid = tx.txid, nxtRsAddress = outputPort.nxtRsAddress, source = Some(outputPort.currency.toString)))
    } else {
