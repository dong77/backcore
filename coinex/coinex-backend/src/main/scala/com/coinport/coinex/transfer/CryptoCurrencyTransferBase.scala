...
trait CryptoCurrencyTransferWithdrawalLikeBase extends CryptoCurrencyTransferBase {
  ...
  override def innerHandleTx(currency: Currency, tx: CryptoCurrencyTransaction, timestasmp: Option[Long]) {
    tx.ids match {
        ...
        idList foreach {
          ...
          id =>
            if (id2HandlerMap.contains(id)) {
              tx.status match {
                case Failed =>
                  handleFailed(id2HandlerMap(id).setTimeStamp(timestasmp))
                case _ =>
                  id2HandlerMap(id).setTimeStamp(timestasmp).onNormal(tx)
              }
            } else {
