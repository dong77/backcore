...
trait TransactionMongoHandler {
  ...
  private def toBson(item: TransactionItem) = {
    ...
    MongoDBObject(
      ...
      MARKET -> side.market.toString, SIDE -> side.ordered, TIMESTAMP -> item.timestamp)
  }
  ...
  private def toClass(obj: MongoDBObject) = TransactionItem(
    ...
    side = obj.getAsOrElse(MARKET, "").getMarketSide(obj.getAsOrElse(SIDE, true)),
  ...
  private def mkQuery(q: QueryTransaction): MongoDBObject = {
    ...
    if (q.side.isDefined) query = {
      ...
      if (querySide.bothSide) query ++ (MARKET -> market.toString)
      else query ++ (MARKET -> querySide.side.market.toString, SIDE -> querySide.side.ordered)
    }
    query
  }
}
