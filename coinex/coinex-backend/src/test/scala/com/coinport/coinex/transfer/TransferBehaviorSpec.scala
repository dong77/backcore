...
class TransferBehaviorSpec extends EmbeddedMongoForTestWithBF {
  ...
  "AccountTransferWithdrawSpec" should {
    ...
    "be able to save transferHandler and query them" in {
      ...
      val d3 = AccountTransfer(id = 3, userId = 2, `type` = TransferType.Deposit, currency = Currency.Rmb, amount = 1000, created = Some(300), updated = Some(800))
      ...
      q = QueryTransfer(uid = Some(1), currency = Some(Currency.Rmb), cur = Cursor(0, 10), getCount = true)
      dw.transferHandler.count(dw.transferHandler.getQueryDBObject(q)) should be(1)

      q = QueryTransfer(uid = Some(1), currency = Some(Currency.Rmb), cur = Cursor(0, 10), getCount = false)
