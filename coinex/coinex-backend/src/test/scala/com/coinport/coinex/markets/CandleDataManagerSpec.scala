  ...
  "ChartDataManagerSpec" should {
    ...
    "save data from order submitted" in {
      ...
      manager.getCandleItems(OneMinute, 800000, 1200000) mustEqual
        Seq(CandleDataItem(1000000, 1, 3000, 1 / 3000.0, 1 / 3000.0, 1 / 3000.0, 1 / 3000.0, market))
    }
    ...
    "can fill empty candle by transaction" in {
      ...
      val rv = manager.getCandleItems(OneMinute, c1, c1 + 30 * 60 * 1000 * 2)
      rv.size mustEqual 31
      (0 until rv.size).foreach { i =>
        if (i < 30) rv.apply(i).open mustEqual (1.0 / 3000)
        else rv.apply(i).open mustEqual (1.0 / 2000)
      }

      val rv2 = manager.getCandleItems(OneDay, c1, c1 + 30 * 60 * 1000 * 2)
    ...
    "can fill empty candle by query" in {
      ...
      manager.query(OneMinute, c1, c1 + 30 * 60 * 1000 * 2) mustEqual Nil
