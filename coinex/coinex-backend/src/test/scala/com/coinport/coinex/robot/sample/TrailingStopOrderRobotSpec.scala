  ...
  "stop order robot" should {
    "trailing stop order robot for sell btc without trailing" in {
      ...
      val (robot3, res3) = robot2.action(
        Some(RobotMetrics(Map((Btc ~> Rmb) -> MarketByMetrics((Btc ~> Rmb), 3601.0))))
      )
      res3 mustEqual None
    ...
    "trailing stop order robot for sell btc with trailing" in {
      ...
      res3 mustEqual None
      val (robot4, res4) = robot3.action(
        Some(RobotMetrics(Map((Btc ~> Rmb) -> MarketByMetrics((Btc ~> Rmb), 5000.0))))
      )
      res4 mustEqual None
      val (robot5, res5) = robot4.action(
        Some(RobotMetrics(Map((Btc ~> Rmb) -> MarketByMetrics((Btc ~> Rmb), 4501.0))))
      )
      res5 mustEqual None
    ...
    "trailing stop order robot for buy btc without trailing" in {
      ...
      res2 mustEqual None
      val (robot3, res3) = robot2.action(
        Some(RobotMetrics(Map((Rmb ~> Btc) -> MarketByMetrics((Rmb ~> Btc), 1 / 4999.0))))
      )
      res3 mustEqual None
      val (robot4, res4) = robot3.action(
    ...
    "trailing stop order robot for buy btc with trailing" in {
      ...
      res2 mustEqual None
      val (robot3, res3) = robot2.action(
        Some(RobotMetrics(Map((Rmb ~> Btc) -> MarketByMetrics((Rmb ~> Btc), 1 / 4500.0))))
      )
      res3 mustEqual None
      val (robot4, res4) = robot3.action(
        Some(RobotMetrics(Map((Rmb ~> Btc) -> MarketByMetrics((Rmb ~> Btc), 1 / 2000.0))))
      )
      res4 mustEqual None
      val (robot5, res5) = robot4.action(
        Some(RobotMetrics(Map((Rmb ~> Btc) -> MarketByMetrics((Rmb ~> Btc), 1 / 2499.0))))
      )
      res5 mustEqual None
      val (robot6, res6) = robot5.action(
